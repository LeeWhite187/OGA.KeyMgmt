...
namespace NETCore_Common_Tests
{
    ...
    /// </summary>
    [TestClass]
    public class KeyStore_v2_JsonConfig_Tests
    {
        // Test  1  Simple construction of KeyStore JsonConfig v2 with storage password.
        ...
        public void Test01()
        {
            ...
            if (ks.KeyStoreType != "Json")
                Assert.Fail("Instance is not correct.");
            if (ks.KeyStoreVersion != 2)
                Assert.Fail("Instance is not correct.");
        }

        // Test  1a Simple construction of KeyStore JsonConfig v2 with storage key.
        //          Was constructed without exception.
        [TestMethod]
        public void Test01a()
        {
            KeyObject_v2 sk = null;
            var res = KeyStore_v2_Base.Create_New_AES_Key(Guid.NewGuid().ToString(), 256, out sk);
            if (res != 1)
                Assert.Fail("Failed to create storage key.");

            var ks = new KeyStore_v2_JsonConfig(sk);
        ...
        // Test  2  Simple construction of KeyStore File v2 with storage key.
        //          Was constructed without exception.
        [TestMethod]
        public void Test02()
        {
            KeyObject_v2 storagekey = null;
            var ressto = KeyStore_v2_Base.Create_New_AES_Key(Guid.NewGuid().ToString(), 256, out storagekey);
            if (ressto != 1)
                Assert.Fail("Failed to create storage key.");

            var ks = new KeyStore_v2_JsonConfig(storagekey);
        ...
        public void Test10()
        {
            ...
            int res = KeyStore_v2_JsonConfig.Create_New_AES_Key(keyname, 256, out var k1);
            if (res != 1)
        ...
        public void Test12()
        {
            ...
            var res2 = KeyStore_v2_JsonConfig.Verify_AESKey(k2);
        ...
        public void Test13()
        {
            ...
            // Now, call the verification method...
            var res2 = KeyStore_v2_JsonConfig.Verify_AESKey(k1);
        ...
        public void Test14()
        {
            ...
            // Call the creation method...
            int res = KeyStore_v2_JsonConfig.Create_New_RSA_KeyPair(keyname, 511, out var k1);
            if (res != -1)
        ...
        public void Test15()
        {
            ...
            // Call the creation method...
            int res = KeyStore_v2_JsonConfig.Create_New_RSA_KeyPair(keyname, 1024, out var k1);
            if (res != 1)
                Assert.Fail("Failed to create key");

            // Now, call the verification method...
            var res2 = KeyStore_v2_JsonConfig.Verify_RSAKeyPair(k1);
            if(res2 != 1)
                Assert.Fail("Failure");
        }
        ...
        // Test 17  Create an RSA key pair with a privatekey encryption password. Attempt usage to ensure we can perform signing and verification.
        //          Check that the key usage worked.
        [TestMethod]
        public void Test17()
        {
        ...
        public void Test19()
        {
            ...
            // Call the creation method...
            int res = KeyStore_v2_JsonConfig.Create_New_ECDSA_KeyPair(keyname, out var k1);
            if (res != 1)
                Assert.Fail("Failed to create key");

            // Now, call the verification method...
            var res2 = KeyStore_v2_JsonConfig.Verify_ECDSAKeyPair(k1);
            if(res2 != 1)
                Assert.Fail("Failure");
        }
        ...
        // Test 21  Create an ECDSA key pair. Attempt usage to ensure we can perform signing and verification.
        //          Check that the key usage worked.
        [TestMethod]
        public void Test21()
        {
        ...
        // Test 22  Do a naked EC key creation, then a sign with a new key instance, then a verify with a third new instance.
        //          Check that the key works.
        [TestMethod]
        public void Test22()
        {
        ...
        public void Test23()
        {
            ...
            // Create an ec key instance...
            var res44 = KeyStore_v2_JsonConfig.Create_New_ECDSA_KeyPair(keyname, out var k1);
        ...
        // Test 30  Create a key, add it to the store. Save the store to a json config instance.
        //          Load the store from config instance, retrieve the key.
        ...
        public void Test31()
        {
            ...
            // Save the store to disk...
            var saveres = ks.Save(out var cfg);
            if(saveres != 1)
                Assert.Fail("Problem with saving.");
        ...
        public void Test32()
        {
            ...
            var save2res = ks2.Save(out var cfg2);
            if(save2res != 1)
                Assert.Fail("Problem with saving.");
            ...
            var ks3 = new KeyStore_v2_JsonConfig(storagepassword);



            // Have the third keystore load keys...
            var lkres3 = ks3.Load(cfg2);
            if(lkres3 != 1)
                Assert.Fail("Problem with loading.");
